...
lib.create = (dir, file, data, callback) => {
  ...
  fs.open(`${lib.basedir + dir}/${file}.json`, "wx", (err, fileDiscriptor) => {
    if (!err && fileDiscriptor) {
      ...
      fs.write(fileDiscriptor, stringData, (writeErr) => {
        if (!writeErr) {
          fs.close(fileDiscriptor, (closeErr) => {
            if (!closeErr) {
              callback(false);
            } else {
...
lib.update = (dir, file, data, callback) => {
  fs.open(`${lib.basedir + dir}/${file}.json`, "r+", (err, fileDiscriptor) => {
    if (!err && fileDiscriptor) {
      ...
      fs.ftruncate(fileDiscriptor, (truncErr) => {
        if (!truncErr) {
          // write to the file and close it
          fs.writeFile(fileDiscriptor, stringData, (writeErr) => {
            if (!writeErr) {
              // close file
              fs.close(fileDiscriptor, (closeErr) => {
                if (!closeErr) {
